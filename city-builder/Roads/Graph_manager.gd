...
func _split_existing_roads_at_junctions(current_juncs: Array):
	for j_pos in current_juncs:
		...
		# 1. Primero buscamos qué carreteras pasan por esa posición 2D
		for r in roads:
			if r.start.position == j_pos or r.end.position == j_pos: continue
			
			var closest = Geometry2D.get_closest_point_to_segment(j_pos, r.start.position, r.end.position)
			
			if closest.distance_to(j_pos) < 2.0:
				# --- AQUÍ ESTÁ EL TRUCO ---
				# Calculamos la altura de la carretera existente en ese punto
				var altura_carretera = get_height_at_point(r, j_pos)
				
				# Buscamos si el "nodo de la intersección" tiene una altura similar
				# Para esto, comparamos con la altura que debería tener según el dibujo actual
				# Si la diferencia de altura es mayor a 2 metros, NO la metemos en roads_to_split
				var altura_nueva = 0.0 # Valor por defecto
				
				# Buscamos el nodo que acabamos de crear en detect_junction para ver su altura
				for n in nodes:
					if n.position.distance_to(j_pos) < 1.0:
						altura_nueva = n.height_in_m
						break
				
				if abs(altura_carretera - altura_nueva) < 1.0:
					roads_to_split.append(r)
				
		# 2. Solo cortamos las carreteras que están a la misma altura
		for r in roads_to_split:
			var old_start = r.start
			var old_end = r.end
			var nivel_original = old_start.level # Guardamos el nivel original
			var altura_original = get_height_at_point(r, j_pos)

			roads.erase(r)
			old_start.roads.erase(r)
			old_end.roads.erase(r)
			
			# Creamos el nodo manteniendo la altura de la carretera que estamos cortando
			var j_node = get_or_create_node(j_pos, nivel_original)
			j_node.height_in_m = altura_original
			m_to_level(j_node) 
